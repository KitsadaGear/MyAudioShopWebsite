<?php 
    ...
    if (isset($_POST['change_password'])) {
        ...
        if (count($errors) == 0) {
            ...
            $query = "SELECT * FROM user_information WHERE username = '$username' AND password = '$password' AND email = '$email' ";
            $result = mysqli_query($conn, $query);
            
            if (mysqli_num_rows($result) == 1) {
                $change_password = "UPDATE user_information SET password = '$new_password'" ; 
